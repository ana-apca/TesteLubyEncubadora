...
WHERE id NOT IN (SELECT E.pessoa_id
					FROM tabela_evento E);
...
(
    ...
    telefone varchar(200),
    pessoa_id integer,
    CONSTRAINT PK_tabela_telefone PRIMARY KEY (id),
    CONSTRAINT FK_tabela_pessoa FOREIGN KEY (pessoa_id) REFERENCES tabela_pessoa (id)    
);    

-- 2.9 Crie uma query para criar uma índice do tipo único na coluna telefone da 'tabela_telefone'.
CREATE UNIQUE INDEX IDX_Telefone   
